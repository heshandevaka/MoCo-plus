...
_parser.add_argument('--weighting', type=str, default='EW',
    help='loss weighing strategies, option: EW, UW, GradNorm, GLS, RLW, \
        MGDA, PCGrad, GradVac, CAGrad, GradDrop, DWA, IMTL, MoCo, MoDo, ITL, EW_STORM') # ADDED MoCo, MoDo, and PW
...
## MoCo
_parser.add_argument('--beta_moco', type=float, default=0.1, help='learning rate of tracking variable')
_parser.add_argument('--gamma_moco', type=float, default=0.1, help='learning rate of lambda')
_parser.add_argument('--rho_moco', type=float, default=0.0, help='regularization parameter of lambda subproblem')
_parser.add_argument('--moco_gn', default='none', type=str, 
                    help='type of gradient normalization for MoCo, option: l2, none, loss, loss+')
## MoDo
_parser.add_argument('--gamma_modo', type=float, default=0.1, help='learning rate of lambda')
_parser.add_argument('--rho_modo', type=float, default=0.0, help='regularization parameter')
...
## EW_STORM
_parser.add_argument('--c_ew_storm', type=float, default=0.1, help='c parameter in STORM')
_parser.add_argument('--k_ew_storm', type=float, default=0.1, help='k parameter in STORM')
_parser.add_argument('--w_ew_storm', type=float, default=0.1, help='w parameter in STORM')
_parser.add_argument('--sigma_ew_storm', type=float, default=0.1, help='variance parameter in STORM')
## MoCo+
_parser.add_argument('--c_mocoplus', type=float, default=0.1, help='c parameter in MoCo+')
_parser.add_argument('--k_mocoplus', type=float, default=0.1, help='k parameter in MoCo+')
_parser.add_argument('--w_mocoplus', type=float, default=0.1, help='w parameter in MoCo+')
_parser.add_argument('--sigma_mocoplus', type=float, default=0.1, help='variance parameter in MoCo+')
_parser.add_argument('--gamma_mocoplus', type=float, default=0.1, help='lambda stepsize for MoCo+')
_parser.add_argument('--gn_mocoplus', type=str, default="l2", help='y sequence normalization for MoCo+')
...
def prepare_args(params):
    ...
    if params.weighting in ['EW', 'UW', 'GradNorm', 'GLS', 'RLW', 'MGDA', 'IMTL',
                            ...
                            'MoCo', 'MoDo', 'ITL', 'EW_STORM', 'MoCoPlus']:
        ...
        if params.weighting in ['DWA']:
            ...
            if params.beta_moco is not None and params.gamma_moco is not None and params.rho_moco is not None:
                kwargs['weight_args']['beta_moco'] = params.beta_moco
                kwargs['weight_args']['gamma_moco'] = params.gamma_moco
                kwargs['weight_args']['rho_moco'] = params.rho_moco
                if params.moco_gn in ['none', 'l2', 'loss', 'loss+']:
                    kwargs['weight_args']['moco_gn'] = params.moco_gn
                else:
                    raise ValueError('No support moco_gn {} for MoCo'.format(params.moco_gn)) 
            else:
                raise ValueError('MoCo needs beta_moco, gamma_moco, rho_moco, and moco_gn')
        ...
        elif params.weighting in ['ITL']: # TODO: Handle arbitrary preference weights
            if params.task_idx is not None:
                kwargs['weight_args']['task_idx'] = params.task_idx
            else:
                raise ValueError('ITL needs task_idx')
        elif params.weighting in ['EW_STORM']:
            if params.c_ew_storm is not None and params.k_ew_storm is not None and params.w_ew_storm is not None and params.sigma_ew_storm is not None:
                kwargs['weight_args']['c_ew_storm'] = params.c_ew_storm
                kwargs['weight_args']['k_ew_storm'] = params.k_ew_storm
                kwargs['weight_args']['w_ew_storm'] = params.w_ew_storm
                kwargs['weight_args']['sigma_ew_storm'] = params.sigma_ew_storm
                if params.modo_gn in ['none', 'l2', 'loss', 'loss+']:
                    kwargs['weight_args']['modo_gn'] = params.modo_gn
                else:
                    raise ValueError('No support modo_gn {} for MoDo'.format(params.modo_gn)) 
            else:
                raise ValueError('EW_STORM needs c_ew_storm, k_ew_storm, w_ew_storm, and sigma_ew_storm')
        elif params.weighting in ['MoCoPlus']:
            if params.c_mocoplus is not None and params.k_mocoplus is not None and params.w_mocoplus is not None and params.sigma_mocoplus is not None and params.gamma_mocoplus is not None and params.gn_mocoplus is not None:
                kwargs['weight_args']['c_mocoplus'] = params.c_mocoplus
                kwargs['weight_args']['k_mocoplus'] = params.k_mocoplus
                kwargs['weight_args']['w_mocoplus'] = params.w_mocoplus
                kwargs['weight_args']['sigma_mocoplus'] = params.sigma_mocoplus
                kwargs['weight_args']['gamma_mocoplus'] = params.gamma_mocoplus
                if params.gn_mocoplus in ['none', 'l2', 'loss', 'loss+']:
                    kwargs['weight_args']['gn_mocoplus'] = params.gn_mocoplus
                else:
                    raise ValueError('No support gn_mocoplus {} for MoCo+'.format(params.gn_mocoplus))
            else:
                raise ValueError('MoCo+ needs c_mocoplus, k_mocoplus, w_mocoplus, sigma_mocoplus, gamma_mocoplus, and gn_mocoplus')
    else:
